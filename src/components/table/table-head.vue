<template>
    <table cellspacing="0" cellpadding="0" border="0" :style="styles">
        <colgroup>
            <col v-for="(column, index) in columns" :width="setCellWidth(column, index, true, data)">
        </colgroup>
        <thead>
            <tr>
                <th v-for="(column, index) in columns" :class="alignCls(column)">
                    <div :class="cellClasses(column)">
                        ...
                        <template v-else-if="column.type === 'selection'"><Checkbox :disabled="isDisabledAll" :value="isSelectAll" @on-change="selectAll"></Checkbox></template>
                        <template v-else>
        ...
        computed: {
            ...
            isSelectAll () {
                ...
                for (let i = 0; i < this.data.length; i++) {
                    if (!this.objData[this.data[i]._index]._isDisabled && !this.objData[this.data[i]._index]._isChecked) {
                        isSelectAll = false;
                        break;
                    }
                }
            ...
            isDisabledAll () {
                ...
                if (!this.data.length) isDisabledAll = false;
                //if (!this.data.find(item => !item._disabled)) isDisabledAll = false;    // #1751
                for (let i = 0; i < this.data.length; i++) {
